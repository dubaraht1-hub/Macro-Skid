...
public final class GlyphPageFontRenderer {
	...
	private void renderStringAtPos(MatrixStack matrices, CharSequence text, boolean shadow, int color) {
		...
		for (int i = 0; i < text.length(); ++i) {
			...
			if (c0 == '\u00A7' && i + 1 < text.length()) { // Fixed syntax error
	...
	private void doDraw(float f, GlyphPage glyphPage) {
		if (this.strikethroughStyle) {
			...
			bufferBuilder.vertex(this.posX, this.posY + (float) (glyphPage.getMaxGlyphHeight() / 2), 0.0F); // Fixed method name
			bufferBuilder.vertex(this.posX + f, this.posY + (float) (glyphPage.getMaxGlyphHeight() / 2), 0.0F); // Fixed method name
			bufferBuilder.vertex(this.posX + f, this.posY + (float) (glyphPage.getMaxGlyphHeight() / 2) - 1.0F, 0.0F); // Fixed method name
			bufferBuilder.vertex(this.posX, this.posY + (float) (glyphPage.getMaxGlyphHeight() / 2) - 1.0F, 0.0F); // Fixed method name
		...
		if (this.underlineStyle) {
			...
			bufferBuilder.vertex(this.posX + (float) l, this.posY + (float) glyphPage.getMaxGlyphHeight(), 0.0F); // Fixed method name
			bufferBuilder.vertex(this.posX + f, this.posY + (float) glyphPage.getMaxGlyphHeight(), 0.0F); // Fixed method name
			bufferBuilder.vertex(this.posX + f, this.posY + (float) glyphPage.getMaxGlyphHeight() - 1.0F, 0.0F); // Fixed method name
			bufferBuilder.vertex(this.posX + (float) l, this.posY + (float) glyphPage.getMaxGlyphHeight() - 1.0F, 0.0F); // Fixed method name
			BufferRenderer.drawWithGlobalProgram(bufferBuilder.end());
		}
	...
	public int getFontHeight() {
		return regularGlyphPage.getMaxGlyphHeight() / 2; // Fixed method name
	}
	...
	public int getStringWidth(CharSequence text) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (character == '\u00A7') // Fixed syntax error
				on = true;
